    ...
    public class AlumnoDAL
    {
        ...
        internal bool ValidarAlumnoNotificado(string username)
        {
            using (dev_Uni_Entities dev_Uni_Entities = new dev_Uni_Entities())
            {
                ...
                {
                    sp_uni_get_alumno_carrera_idEntidad_Result alumnoCarrera = dev_Uni_Entities.sp_uni_get_alumno_carrera_idEntidad(alumnosDatosPersonales.IdEntidad).FirstOrDefault();
                    if(alumnoCarrera != null)
                    {
                        if(alumnoCarrera.Notificado != null)
                        {
                            return alumnoCarrera.Notificado;
                        }
                        return false;
                    }
                }
        ...
        internal void UpdateAlumnoNotificado(string username)
        {
            using (dev_Uni_Entities dev_Uni_Entities = new dev_Uni_Entities())
            {
                ...
                if (alumnosDatosPersonales != null)
                {
                    List<uniAlumno> uniAlumnoList = dev_Uni_Entities.uniAlumnos.Where(x => x.IdEntidad == alumnosDatosPersonales.IdEntidad).ToList();
                    foreach (uniAlumno uniAlumno in uniAlumnoList)
                    {
                        uniAlumno.Notificado = true;
                        dev_Uni_Entities.SaveChanges();
                    }
                }
            }
